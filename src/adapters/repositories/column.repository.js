...
export class ColumnRepository {
  ...
  async findColumnById(columnId) {
    ...
    return this.prisma.cmsColumn.findUnique({
      where: { id: columnId },
      include: {
        table: true,
        cells: true,
  ...
  async findColumnsByTableId(tableId) {
    ...
    return this.prisma.cmsColumn.findMany({
      where: { tableId },
      include: {
        table: true,
        cells: {
  ...
  async updateColumn(columnId, data) {
    ...
    return this.prisma.cmsColumn.update({
      where: { id: columnId },
      data,
      include: {
        table: true,
  ...
  async deleteColumn(columnId) {
    ...
    return this.prisma.cmsColumn.delete({
      where: { id: columnId },
    });
  }
  ...
  async checkColumnOwnership(columnId, userId) {
    ...
    const column = await this.prisma.cmsColumn.findUnique({
      where: { id: columnId },
      include: {
        table: {
          include: {
