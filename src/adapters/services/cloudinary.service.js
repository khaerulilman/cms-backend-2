import cloudinaryClient from '../../frameworks/config/cloudinary.js';
...
export class CloudinaryService {
  ...
  static async duplicateImage(imageUrl) {
    try {
      ...
      const result = await cloudinaryClient.uploader.upload(imageUrl, {
        folder: 'cms-uploads',
        resource_type: 'auto',
      });
  ...
  static async deleteImages(publicIds) {
    try {
      ...
      const results = await Promise.all(
        publicIds.map((publicId) => cloudinaryClient.uploader.destroy(publicId)),
      );
      logger.info(
        { count: publicIds.length },
