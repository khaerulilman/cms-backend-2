import { SUCCESS_MESSAGES, ERROR_MESSAGES } from '../../entities/constants/http.js';
import logger from '../../frameworks/logging/logger.js';

export class RowController {

  constructor({ rowUseCase }) {
    this.useCase = rowUseCase;
  }
  ...
  async createRow(req, res, next) {
    try {
      ...
      const row = await this.useCase.createRow(tableId, userId);
  ...
  async getRowsByTable(req, res, next) {
    try {
      ...
      const rows = await this.useCase.getRowsByTable(tableId, userId);
  ...
  async getRowById(req, res, next) {
    try {
      ...
      const row = await this.useCase.getRowById(rowId, userId);
  ...
  async updateRow(req, res, next) {
    try {
      ...
      const updatedRow = await this.useCase.updateRow(rowId, userId, data);
  ...
  async deleteRow(req, res, next) {
    try {
      ...
      const deletedRow = await this.useCase.deleteRow(rowId, userId);
  ...
  async bulkDeleteRows(req, res, next) {
    try {
      ...
      const result = await this.useCase.deleteRows(rowIds, userId);
